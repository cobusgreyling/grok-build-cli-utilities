...
### Added
- **`usage cost` / `usage report --all`**: print every bucket. Default `--top` is 10 by list$ (cost was 8). Truncated tables say `top 10 of 18 · TOTALS is the whole window · --all to see all`. `--all` overrides `--top`. TOTALS is the whole window. Hidden rows stay uncombined.
- **`usage cost --by session`** and **`--by pr`**: `--by session` is one row per chat. `--by pr` is one row per GitHub PR when that chat created exactly one PR in the date window (`repo#N`, issue from `Fixes` or `Closes`). Several PRs from one parent chat stay one unsplit row (`widgets #12,15` or mixed `widgets #7,8,14 · notes #15`). A one-line footnote marks unsplit multi-PR rows. No GitHub owner. No session UUID in the key unless two rows share a display Key (`notes#22 · 01a05e3c…`). Unlabeled `--by session` Keys pretty-print `*-issue-N` clones (`notes-issue-9` → `notes#9`) and Grok worktrees. Sessions with no PR create in the `--from`/`--to` window are omitted from `--by pr` unless `--include-unlabeled`. Chat text and `get_pull_request` are ignored. PR-level Keys need github `create_pull_request` or `gh pr create` in `updates.jsonl`, with session cwd in that repo.
...
### Fixed
- **`usage cost --by pr` Keys ignored `--from`/`--to`**: Keys now use the same local date window as token totals. A chat that opened PRs on an earlier day is omitted from `--by pr` unless `--include-unlabeled` (pretty Key, not older PRs). `--by session` still lists the chat; Key is in-window PRs or a pretty cwd. `--by app` is unchanged. The empty warning `No created PRs in this window` means no creates in the window.
...
### Changed
- **`usage cost --by app`**: repo inferred from cwd. Issue worktrees (`repo-issue-N`) and Grok worktrees (`~/.grok/worktrees/github-…`) roll into one Key per repo. Grouping is case-insensitive. Display prefers the `GitHub/` folder spelling. `#N` is gone from `--by app` for issue clones. `--by pr` Keys are creates in the date window. `--by project` is still the full cwd. Unlabeled `--by session` Keys still pretty-print `repo#N` / `(worktree)` so chats stay distinct.
- **`usage cost` default `--top`** is 10, matching `usage report`. `--top 0` prints no rows (not all rows).
- **`usage --from` / `--to` / `--since`** are inclusive **local** calendar dates by default (same clock as weekly resets / Build `/usage`). Late-evening local work stays on that local day. `--tz UTC` or `--tz America/New_York` (IANA) and `[usage] date_tz` are the escape hatch. `--by day` buckets use the same zone. JSON includes `date_tz`.
